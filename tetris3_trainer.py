# adds better training with comparison to master, titles for games
...
def play_tetris(modify : int, seed : int, title : str = ''):
    ...
    display_text = font.render(title, True, 'white')
    text_rect = display_text.get_rect()
    text_rect.center = (resolution[0]//2, resolution[1] // 24 * 23)
...
# bot brain variables
{'covered': 20.77437875269985, 'height': 10.445111780361902, 'twospace': 13.392648333073485, 'edgespace': 2.6429502538914815}
master = {'covered': 19.012118872997224, 'height': 11.431058837933584, 'twospace': 15.137066797468599, 'edgespace': 3.5613392066284546}
high = (0, master)

# tetris bot trainer
while True:
    
    # creates new generation and selects a winner
    ...
    for i in range(20):
        ...
        score = play_tetris(temp, seed, f'player {i + 1}, high {high[0]}')
        
        if score > high[0]: high = (score, temp)
    
    # current parent and contestant compete
    score = [0, 0]
    for i in range(11):
        
        random.seed()
        seed = random.randint(1, 1000)
        master_score = play_tetris(master, seed, f'p {score[0]} - {score[1]} c')
        child_score = play_tetris(high[1], seed, f'p {score[0]} - {score[1]} c')
        
        # master wins = positive, child wins = negative
        if master_score > child_score: score[0] += 1
        else: score[1] += 1
        
        if score[0] == 6 or score[1] == 6: break
